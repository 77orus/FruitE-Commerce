import 'package:flutter/material.dart';

class TextStyles {
  // حجم 13
  static TextStyle regular13 = const TextStyle(
    fontSize: 13,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold13 = const TextStyle(
    fontSize: 13,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold13 = const TextStyle(
    fontSize: 13,
    fontWeight: FontWeight.w600,
  );

  // حجم 16
  static TextStyle regular16 = const TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold16 = const TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold16 = const TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
  );

  // حجم 18
  static TextStyle regular18 = const TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold18 = const TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold18 = const TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
  );

  // حجم 20
  static TextStyle regular20 = const TextStyle(
    fontSize: 20,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold20 = const TextStyle(
    fontSize: 20,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold20 = const TextStyle(
    fontSize: 20,
    fontWeight: FontWeight.w600,
  );

  // حجم 23
  static TextStyle regular23 = const TextStyle(
    fontSize: 23,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold23 = const TextStyle(
    fontSize: 23,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold23 = const TextStyle(
    fontSize: 23,
    fontWeight: FontWeight.w600,
  );

  // حجم 26
  static TextStyle regular26 = const TextStyle(
    fontSize: 26,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold26 = const TextStyle(
    fontSize: 26,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold26 = const TextStyle(
    fontSize: 26,
    fontWeight: FontWeight.w600,
  );

  // حجم 28
  static TextStyle regular28 = const TextStyle(
    fontSize: 28,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold28 = const TextStyle(
    fontSize: 28,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold28 = const TextStyle(
    fontSize: 28,
    fontWeight: FontWeight.w600,
  );

  // حجم 30
  static TextStyle regular30 = const TextStyle(
    fontSize: 30,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold30 = const TextStyle(
    fontSize: 30,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold30 = const TextStyle(
    fontSize: 30,
    fontWeight: FontWeight.w600,
  );

  // حجم 34
  static TextStyle regular34 = const TextStyle(
    fontSize: 34,
    fontWeight: FontWeight.normal,
  );

  static TextStyle bold34 = const TextStyle(
    fontSize: 34,
    fontWeight: FontWeight.bold,
  );

  static TextStyle semiBold34 = const TextStyle(
    fontSize: 34,
    fontWeight: FontWeight.w600,
  );
}
